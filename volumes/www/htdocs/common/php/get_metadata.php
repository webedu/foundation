...
function get_metadata ($lom_file) {
		...
		$metadata['title'] = array('label'=>'Titel', 'data'=>$lom_xml->general->title->children()->__toString());		
		...
		foreach ($lom_xml->lifeCycle->contribute as $contribute) {
			if ($contribute->role->value[0]=="author") {
				...
				foreach($contribute->entity as $author) {
					...
					$authors[]=$author_title.(!empty($author_title)?" ": "").$author_name;
				}
				$metadata['author'] = array('label'=>'Autor(in)', 'data'=>implode(", ",$authors));
                                if(!is_null($contribute->date->dateTime)) {
		...
		foreach ($lom_xml->lifeCycle->contribute as $contribute) {
			if ($contribute->role->value[0]=="unknown") {
				...
				foreach($contribute->entity as $co_author) {
					...
					$co_authors[]=$author_title.(!empty($author_titl)?" ": "").$author_name;
				}
		...
		foreach ($lom_xml->relation as $relation) {
			...
			//
                        if (!empty($relation->resource->description->string)) {
		...
		$cat_0=array();
		foreach ($lom_xml->relation as $relation) {
                    if(!is_null($relation->resource->identifier->catalog)) {
		...
		$metadata['keywords'] = array('label'=>'Schlagw̦rter',	'data'=>implode("<br/>\n",$keywords));
